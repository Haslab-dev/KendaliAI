...
import {
  X, Zap, Bot, MessageSquare, Plug, Brain, Wrench, Smartphone, Plus, Trash2, CheckCircle,
...
export const ManagementCenter: React.FC = () => {
  const { activeModal, setActiveModal, loadAgents, loadSessions, selectSession: chooseSession } = useAppStore() as any;

  const [activeTab, setActiveTab] = useState<string>(activeModal || 'providers');

  useEffect(() => {
    if (activeModal) setActiveTab(activeModal);
  }, [activeModal]);
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#171717] dark:bg-[#171717] light:bg-[#f7f7f8] border border-[#2e2e2e] rounded-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#262626]">
          <div className="flex items-center gap-2">
            <span className="text-xl">🪶</span>
            <h2 className="text-base font-bold text-neutral-100">KendaliAI Gateway Control Center</h2>
          </div>
          <button
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Tabs Sidebar */}
          <div className="w-56 bg-[#121212] border-r border-[#262626] p-3 space-y-1 select-none">
            {[
              ...
              { id: 'sessions', label: 'Session Registry', icon: MessageSquare },
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                    isActive
                      ? 'bg-[#212121] text-blue-400 font-semibold'
          ...
          <div className="flex-1 p-6 overflow-y-auto custom-scrollbar flex flex-col">
            ...
            {activeTab === 'logs' && <LogsStreamingView onClose={() => setActiveModal(null)} />}
            {activeTab === 'sessions' && <SessionsTab onSelectSession={(id) => { setActiveModal(null); chooseSession(id); }} />}
            {activeTab === 'docs' && <DocStoreTab onChatWithDoc={(docTitle) => { setActiveModal(null); }} />}
...
// 3. Sessions Tab
const SessionsTab: React.FC<{ onSelectSession: (id: string) => void }> = ({ onSelectSession }) => {
...
const DocStoreTab: React.FC<{ onChatWithDoc: (docTitle: string) => void }> = ({ onChatWithDoc }) => {
  const { createSession, setActiveModal } = useAppStore() as any;
