...
describe("Telegram Channel Smoke Tests", () => {
  ...
  describe("Bot Initialization", () => {
    test(
      "should authenticate with valid bot token",
      async () => {
    ...
    test(
      "should have bot capabilities info",
      async () => {
        const me = await client.apiCall<TelegramBotInfo>("getMe");
  ...
  describe("Invalid Token Handling", () => {
    test(
      "should reject invalid bot token",
      async () => {
  ...
  describe("Webhook Management", () => {
    test(
      "should get webhook info",
      async () => {
        const webhookInfo =
          await client.apiCall<TelegramWebhookInfo>("getWebhookInfo");
    ...
    test(
      "should be able to delete webhook",
      async () => {
    ...
    test(
      "should have empty webhook URL after deletion",
      async () => {
        const webhookInfo =
          await client.apiCall<TelegramWebhookInfo>("getWebhookInfo");
  ...
  describe("Updates", () => {
    test(
      "should get updates without error",
      async () => {
    ...
    test(
      "should handle allowed_updates parameter",
      async () => {
        const updates = await client.apiCall<TelegramUpdate[]>("getUpdates", {
          limit: 5,
          timeout: 0,
  ...
  describe("Chat Actions", () => {
    ...
    (chatId ? test : test.skip)(
      "should send a message to a chat",
      async () => {
        const result = await client.apiCall<{ message_id: number }>(
          "sendMessage",
          {
            chat_id: chatId,
    ...
    (chatId ? test : test.skip)(
      ...
      async () => {
        ...
        const sent = await client.apiCall<{ message_id: number }>(
          "sendMessage",
          {
            chat_id: chatId,
  ...
  describe("Me Info", () => {
    test(
      "should have consistent bot info across calls",
      async () => {
