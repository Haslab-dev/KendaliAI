...
describe("DeepSeek Provider Smoke Tests", () => {
  ...
  describe("Chat Completion", () => {
    test(
      "should generate a simple response",
      async () => {
        const options: GenerateOptions = {
          messages: [
            {
              role: "user",
              content: 'Say "Hello, smoke test!" and nothing else.',
            },
          ],
          maxTokens: 50,
          temperature: 0,
  ...
  describe("JSON Mode", () => {
    test(
      "should generate valid JSON when requested",
      async () => {
        const options: GenerateOptions = {
          messages: [
            {
              role: "user",
              content:
                'Return a JSON object with keys "name" and "value" where name is "test" and value is 42.',
            },
          ],
          maxTokens: 100,
          temperature: 0,
