...
export function heuristicRouter(message: string): Intent | "unknown" {
  ...
  const chatTriggers = ["hi", "hello", "hey", "how are you", "who are you", "what are you"];
  if (chatTriggers.some(kw => msg === kw || msg.startsWith(kw) || msg.includes("identity"))) {
    return "chat";
  }
  ...
  const ragKeywords = [
    "how do i", "policy", "documentation", "info about",
    "details on", "knowledge base", "our", "tell me about", "docs"
  ];

  // Also "what is" but only if not followed by "identity" or similar
  if (ragKeywords.some(kw => msg.includes(kw)) || (msg.includes("what is") && !msg.includes("your identity"))) {
    return "rag";
  }
