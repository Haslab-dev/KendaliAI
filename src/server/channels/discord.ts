...
export class DiscordChannel extends BaseChannel {
  ...
  protected async doSendMessage(text: string, options?: SendMessageOptions): Promise<ChannelMessage> {
    ...
    const result = await this.apiCall<DiscordMessage>('POST', `/channels/${channelId}/messages`, body);
  ...
  async getChatInfo(chatId: string): Promise<ChatInfo> {
    const channel = await this.apiCall<DiscordChannelData>('GET', `/channels/${chatId}`);
    
    const typeMap: Record<number, 'private' | 'group' | 'supergroup' | 'channel'> = {
  ...
  }, options?: SendMessageOptions): Promise<ChannelMessage> {
    const channelId = options?.chatId || this.config.defaultChatId;
    
    const result = await this.apiCall<DiscordMessage>('POST', `/channels/${channelId}/messages`, {
      embeds: [embed],
    });
   ...
   */
  async replyToInteraction(interactionToken: string, content: string): Promise<void> {
    const applicationId = this.config.botId;
    await this.apiCall(
      'POST',
  ...
  private handleInteraction(interaction: DiscordInteraction): void {
    if (interaction.type === 2 && interaction.data && interaction.user) {
      ...
      const query: CallbackQuery = {
        ...
        message: interaction.message ? this.convertMessage(interaction.message) : undefined,
        userId: interaction.user.id,
        username: interaction.user.username,
        chatId: interaction.channel_id,
      };
