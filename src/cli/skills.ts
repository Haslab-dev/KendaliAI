 ...
 */
export async function handleSkillsCommand(db: Database, args: string[]): Promise<void> {
  ...
  switch (subCommand) {
    ...
    case "ls": {
      ...
      for (const skill of skills) {
        const desc = (skill.description || "No description").slice(0, 45).padEnd(45);
    ...
    case "add": {
      ...
      try {
        ...
        console.log(`✅ Skill '${skill.name}' installed and registered successfully!`);
      } catch (err) {
        console.error(`❌ Failed to install skill: ${err instanceof Error ? err.message : String(err)}`);
      }
      break;
    }
    ...
    case "create": {
      ...
      try {
        ...
        console.log(`\nNext steps:`);
        console.log(`  1. Edit .kendaliai/skills/${name}/SKILL.toml to define tools`);
        console.log(`  2. Implement tools in .kendaliai/skills/${name}/src/index.ts`);
    ...
    case "uninstall": {
      ...
      try {
        ...
        console.error(`❌ Failed to remove skill: ${err instanceof Error ? err.message : String(err)}`);
      }
      break;
    }
